...
"                              \_/\_/ \___/|_|_|
"      asmwolf?
"      badwolf, for AVR assembly (avra)
"
" A Vim colorscheme originally by Steve Losh, adapted for .asm by Mike Gazes.
" Available at {no where yet}.
"
" How to edit asmwolf.vim for .avra files on a 256-color terminal{{{
" Do not use `asm` for `avra` files!{{{
" vim81/syntax already includes `avra.vim` to define the highlight groups:
"   /usr/share/vim/vim81/syntax/avra.vim
" The highlight groups are defined in syntax/avra.vim
" For example:
"   hi def link avraRegister  type
" This is why 'Type' controls the color of registers.
" Out-of-the-box Vim does not recognize when it enters an `.avra` file.
" Make this file so Vim recognizes `filetype=avra` when entering `.avra` files:
"   /home/Mike/.vim/pack/bundle/start/vim-avra/ftdetect/avra.vim
" 'bundle' is your username or whatever, but the other folder names matter
" so: 'pack/foo/bundle'
" This `ftdetect/avra.vim` file contains one line:
"   autocmd BufNewFile,BufRead *.avra set filetype=avra
"}}}
" asm syntax highlighting (abandoned){{{
" Example:
"   c:	0c 94 51 00 	jmp	0xa2	; 0xa2 <__bad_interrupt>
"   `jmp` is an Identifier
"   Unfortunately, the `c` in both `c:` and `0c 94 51 00` also highlights as an
"   'Identifier'.
"}}}
" avra syntax highlighting{{{
" Example:
"   Sections:
"   c:	0c 94 51 00 	jmp	0xa2	; 0xa2 <__bad_interrupt>
"
" - '94 51' and '0xa2' are 'Constant'
" - 'jmp' is a 'Keyword'
" - '; 0xa2 <__bad_interrupt>' is a 'Comment'
" - 'Sections:' and 'c:' are `Label`
" - '0c' and '00' are 'Normal'
"
" Example:
...
" - 'r1' is a 'Type'
"}}}
" `s:bwc` is a dictionary of colors.{{{
" Each color is a list of two values: [guifg, ctermfg]
" Example:
"   let s:bwc.taffy = ['ff2c4b', 196]
" guifg has no effect on my setup.
" I need to change ctermfg.
"}}}
" Color names are passed as arguments in calls to `s:HL()`{{{
" Example:
"   call s:HL('Statement',   'taffy', '', 'bold')
" To check the values used by the `Statement` highlight group,
" enter a window using asmwolf.vim
"   :colorscheme
"   This should return asmwolf
" and run
"   :hi Statement
" To update the asmwolf.vim in use, source file asmwolf.vim.
"}}}
" UI color assignments are in "General/UI" and "Syntax highlighting"{{{
" These sections list what the colornames affect in the UI and the code.
" The easy way to confirm/check these is to directly edit the `s:bwc`
" dictionary, changing the `ctermfg` (the second entry) value in each pair to
" the value 200 (magenta), then sourcing this file, going back to the window
" using this colorscheme, and see what turns magenta.
"}}}
"}}}
" Supporting code -------------------------------------------------------------
" Preamble {{{
...
let g:colors_name = "asmwolf"

if !exists("g:asmwolf_html_link_underline") " {{{
    let g:asmwolf_html_link_underline = 1
endif " }}}

if !exists("g:asmwolf_css_props_highlight") " {{{
    let g:asmwolf_css_props_highlight = 0
endif " }}}
...
" Pure and simple.
" window name fg for non-active windows
let s:bwc.snow = ['ffffff', 15]
" window name fg for active window
let s:bwc.coal = ['000000', 16]

" All of the Gravel colors are based on a brown from Clouds Midnight.
" not used
let s:bwc.brightgravel   = ['d9cec3', 252]
" window vertical splitting line
let s:bwc.lightgravel    = ['998f84', 245]
" comments
let s:bwc.gravel         = ['857f78', 243]
" line numbers
let s:bwc.mediumgravel   = ['666462', 241]
" window name bg for non-active windows
let s:bwc.deepgravel     = ['45413b', 238]
" bg for DiffText
let s:bwc.deepergravel   = ['35322d', 236]
" cursor line bg and Diff line bg
...
function! s:HL(group, fg, ...)
    ...
    " Example:
    " call s:HL('Ignore', 'blackestgravel', '',      '')
    let histring = 'hi ' . a:group . ' '
    " Example: --expanding the above--
    " let histring = 'hi Ignore '
    if strlen(a:fg)
        " Example: --expanding the above--
        " "if strlen(a:fg)" --evaluates to-- "if 14" which is True
        if a:fg == 'fg'
            " Example: --expanding the above--
            " 'blackestgravel' == 'fg' is False
            let histring .= 'guifg=fg ctermfg=fg '
        else
            let c = get(s:bwc, a:fg)
            " Example: --expanding the above--
            " let c = s:bwc.blackestgravel
            " c = ['141413', 232]
            " Explanation:
            " let c = get({list}, {idx})
            " {list} = s:bwc " This is the [s:]cript varible [b]ad[w]olf[c]olor
            " s:bwc is a giant list generated in lines 57-114.
            " list s:bwc contains an item named blackest gravel:
            " s:bwc = {...blackestgravel...}
            " {idx} = a:fg --expands to-- 'blackestgravel'
            let histring .= 'guifg=#' . c[0] . ' ctermfg=' . c[1] . ' '
            " Example: --expanding the above--
            " let histring .= 'guifg=#141413 ctermfg=232
            " histring --evaluates to-- 'hi Ignore guifg=#141413 ctermfg=232 '
        endif
    endif
...
if exists('g:asmwolf_darkgutter') && g:asmwolf_darkgutter
    let s:gutter = 'blackestgravel'
else
    let s:gutter = 'blackgravel'
endif

if exists('g:asmwolf_tabline')
    if g:asmwolf_tabline == 0
        let s:tabline = 'blackestgravel'
    elseif  g:asmwolf_tabline == 1
        let s:tabline = 'blackgravel'
    elseif  g:asmwolf_tabline == 2
        let s:tabline = 'darkgravel'
    elseif  g:asmwolf_tabline == 3
...
" call s:HL('Normal', 'orange', 'blackestgravel')
call s:HL('Normal', 'brightgravel', 'blackestgravel')
...
call s:HL('Statement',   'taffy', '', 'bold')
" avra instructions
call s:HL('Keyword',     'saltwatertaffy', '', 'bold')
call s:HL('Conditional', 'taffy', '', 'bold')
call s:HL('Operator',    'orange', '', 'none')
" avra 'blah:'
call s:HL('Label',       'tardis', '', 'none')
...
" avra numbers
" call s:HL('Constant',  'dress', '', 'bold')
call s:HL('Constant',  'dress')
...
call s:HL('DiffDelete', 'saltwatertaffy', 'coal')
" call s:HL('DiffAdd',    '',     'deepergravel')
call s:HL('DiffAdd',    'saltwatertaffy',     'deepergravel')
" call s:HL('DiffChange', '',     'darkgravel')
call s:HL('DiffChange', '',     'deepgravel')
" call s:HL('DiffText',   'snow', 'deepergravel', 'bold')
" call s:HL('DiffText',   'coal', 'taffy', 'bold')
call s:HL('DiffText',   'taffy', 'coal', 'bold')
" call s:HL('DiffText',   'taffy', 'mediumgravel', 'bold')
...
if g:asmwolf_css_props_highlight
...
if g:asmwolf_html_link_underline
